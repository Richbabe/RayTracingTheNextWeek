...
bool HittableList::BoundingBox(float InTime0, float InTime1, AABB& OutputBox) const
{
    if(Objects.empty())
    {
        return false;
    ...
    for(const auto& Object : Objects)
    {
        if(!Object->BoundingBox(InTime0, InTime1, TempBox))
        {
            continue;
        }

        OutputBox = bFirstBox ? TempBox : SurroundingBox(OutputBox, TempBox);
